...
public class fly : MonoBehaviour 
{
	...
	void Update () 
	{
		...
		if((Input.GetKey(KeyCode.D)||(Input.GetKey(KeyCode.RightArrow)))&&(ariplne.transform.position.x<23))
		{
			ariplne.transform.Translate(Vector3.right* Time.deltaTime * speed,Space.World);
		}
		if((Input.GetKey(KeyCode.A)||(Input.GetKey(KeyCode.LeftArrow)))&&(ariplne.transform.position.x>-23))
		{
			ariplne.transform.Translate(Vector3.left* Time.deltaTime * speed,Space.World);
		}
		if((Input.GetKey(KeyCode.S)||(Input.GetKey(KeyCode.DownArrow)))&&(ariplne.transform.position.y>-16))
		{
